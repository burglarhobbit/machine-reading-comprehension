		...
		for i in range(config.max_para):
			...
			with tf.variable_scope("emb"+str(i)):
				with tf.variable_scope("char"+str(i)):
					...
					self.ch_pr_ = self.ch_pr[:,i*config.para_limit:(i+1)*config.para_limit,:]
				...
				with tf.name_scope("word"+str(i)):
					c_emb = tf.nn.embedding_lookup(self.word_mat, 
						self.c_pr[:,i*config.para_limit:(i+1)*config.para_limit])
		...
		if config.with_passage_ranking:
			...
			for i in range(config.max_para):
				...
				with tf.variable_scope("passage-ranking-attention"+str(i)):
					...
					vj_P = att_vP[:,i*config.para_limit:(i+1)*config.para_limit,:]
