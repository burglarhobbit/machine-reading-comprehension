...
def run_sample_decode(infer_model, infer_sess, model_dir, hparams,
                      summary_writer, src_data, tgt_data, ques_data, feature_data):
  ...
  _sample_decode(loaded_infer_model, global_step, infer_sess, hparams,
                 infer_model.iterator, src_data, tgt_data, ques_data, feature_data,
                 infer_model.src_placeholder, infer_model.ques_placeholder,
                 infer_model.feature_placeholder, infer_model.batch_size_placeholder, summary_writer)
...
def run_internal_eval(
  ...
  dev_ques_file = "%s/%s.%s" % (hparams.prefix_path, "ques_dev", hparams.src)
  dev_feature_file = "%s/%s.%s" % (hparams.prefix_path, "feature_dev", hparams.src)
  dev_eval_iterator_feed_dict = {
      ...
      eval_model.feature_file_placeholder: dev_feature_file,
  }
  ...
  if use_test_set and hparams.test_prefix:
    ...
    test_ques_file = "%s/%s.%s" % (hparams.prefix_path, "ques_test", hparams.src)
    test_feature_file = "%s/%s.%s" % (hparams.prefix_path, "feature_test", hparams.src)
    test_eval_iterator_feed_dict = {
        ...
        eval_model.feature_file_placeholder: test_feature_file,
    }
...
def run_external_eval(infer_model, infer_sess, model_dir, hparams,
  ...
  dev_src_file = "%s.%s" % (hparams.dev_prefix, hparams.src)
  dev_tgt_file = "%s.%s" % (hparams.dev_prefix, hparams.tgt) # not using this
  dev_ground_t_file = "%s/%s.%s" % (hparams.prefix_path, "ground_t_dev", hparams.tgt) # instead, using this (ground truth)
  dev_ques_file = "%s/%s.%s" % (hparams.prefix_path, "ques_dev", hparams.src)
  dev_feature_file = "%s/%s.%s" % (hparams.prefix_path, "feature_dev", hparams.src)
  dev_infer_iterator_feed_dict = {
      ...
      infer_model.feature_placeholder: inference.load_data(dev_feature_file),
  ...
  dev_scores = _external_eval(
      ...
      dev_infer_iterator_feed_dict,
      dev_ground_t_file, # dev_tgt_file
      "dev",
      summary_writer,
  ...
  if use_test_set and hparams.test_prefix:
    test_src_file = "%s.%s" % (hparams.test_prefix, hparams.src)
    test_tgt_file = "%s.%s" % (hparams.test_prefix, hparams.tgt) # not using this
    test_ground_t_file = "%s/%s.%s" % (hparams.prefix_path, "ground_t_test", hparams.tgt) # instead, using this (ground truth)
    test_ques_file = "%s/%s.%s" % (hparams.prefix_path, "ques_test", hparams.src)
    test_feature_file = "%s/%s.%s" % (hparams.prefix_path, "feature_test", hparams.src)
    test_infer_iterator_feed_dict = {
        ...
        infer_model.feature_placeholder: inference.load_data(test_feature_file),
        infer_model.batch_size_placeholder: hparams.infer_batch_size,
    }
    test_scores = _external_eval(
        ...
        infer_model.iterator,
        test_infer_iterator_feed_dict,
        test_ground_t_file, # test_tgt_file
        "test",
        summary_writer,
        save_on_best=False,
        avg_ckpts=avg_ckpts)
...
def run_full_eval(model_dir, infer_model, infer_sess, eval_model, eval_sess,
                  ...
                  sample_ques_data, sample_feature_data, avg_ckpts=False):
  """Wrapper for running sample_decode, internal_eval and external_eval."""
  run_sample_decode(infer_model, infer_sess, model_dir, hparams, summary_writer,
                    sample_src_data, sample_tgt_data, sample_ques_data, sample_feature_data)
...
def train(hparams, scope=None, target_session=""):
  ...
  sample_feature_data = inference.load_data(dev_feature_file)
  ...
  run_full_eval(
      ...
      sample_tgt_data, sample_ques_data, sample_feature_data, avg_ckpts)
  ...
  while global_step < num_train_steps:
    ...
    if global_step - last_eval_step >= steps_per_eval:
      ...
      run_sample_decode(infer_model, infer_sess,
                        ...
                        sample_tgt_data, sample_ques_data, sample_feature_data)
      run_internal_eval(
          eval_model, eval_sess, model_dir, hparams, summary_writer)
  ...
  for metric in hparams.metrics:
    ...
    result_summary, best_global_step, _ = run_full_eval(
        ...
        summary_writer, sample_src_data, sample_tgt_data, sample_ques_data, sample_feature_data)
...
def _sample_decode(model, global_step, sess, hparams, iterator, src_data,
                   tgt_data, ques_data, feature_data, 
                   iterator_src_placeholder, iterator_ques_placeholder, iterator_feature_placeholder,
                   iterator_batch_size_placeholder, summary_writer):
  """Pick a sentence and decode."""
  decode_id = random.randint(0, len(src_data) - 1)
  utils.print_out("  # %d" % decode_id)
  utils.print_out("  #LEN src %d" % len(src_data))
  utils.print_out("  #LEN ques %d" % len(ques_data))
  utils.print_out("  #LEN feature %d" % len(feature_data))
  utils.print_out("  #feature data: 0 %d" % len(feature_data[0]))

  iterator_feed_dict = {
      iterator_src_placeholder: [src_data[decode_id]],
      iterator_ques_placeholder: [ques_data[decode_id]],
      iterator_feature_placeholder: [feature_data[decode_id]],
