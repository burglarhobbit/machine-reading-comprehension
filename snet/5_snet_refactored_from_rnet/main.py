...
from util import get_record_parser, convert_tokens, evaluate, get_batch_dataset, get_dataset, make_dataset_for_ans_syn
...
def test(config):
	...
	with open(config.char_emb_file, "r") as fh:
		char_mat = np.array(json.load(fh), dtype=np.float32)
	if config.test_data == "train":
		with open(config.train_eval_file, "r") as fh, open(config.train_meta, "r") as fh_: # original: test_eval_file
			eval_file = json.load(fh)
			meta = json.load(fh_)
		dataset_file = config.train_file
		batch = get_dataset(config.train_record_file, get_record_parser(
			config, is_test=True), config).make_one_shot_iterator()
		#total = 70260
		total = meta["total"]
	elif config.test_data == "dev":
		with open(config.dev_eval_file, "r") as fh, open(config.dev_meta, "r") as fh_: # original: test_eval_file
			eval_file = json.load(fh)
			meta = json.load(fh_)
		dataset_file = config.dev_file
		batch = get_dataset(config.dev_record_file, get_record_parser(
			config, is_test=True), config).make_one_shot_iterator()
		total = meta["total"]
	elif config.test_data == "test":
		with open(config.test_eval_file, "r") as fh, open(config.test_meta, "r") as fh_: # original: test_eval_file
			eval_file = json.load(fh)
			meta = json.load(fh_)
		dataset_file = config.test_file
		batch = get_dataset(config.test_record_file, get_record_parser(
		config, is_test=True), config).make_one_shot_iterator()
		total = meta["total"]
	...
	model = Model(config, batch, word_mat, char_mat, trainable=False)
	...
	with tf.Session(config=sess_config) as sess:
		...
		for step in tqdm(range(total // config.batch_size + 1)):
			...
			answer_dict_, remapped_dict_, outlier = convert_tokens(
				eval_file, qa_id.tolist(), yp1.tolist(), yp2.tolist())
			qa_id_concat += qa_id.tolist()
			yp1_concat += yp1.tolist()
			yp2_concat += yp2.tolist()
			answer_dict.update(answer_dict_)
		...
		make_dataset_for_ans_syn(dataset_file, eval_file, config.test_data, qa_id_concat,
			yp1_concat, yp2_concat)
