...
def create_train_model(
  ...
  ques_file = "%s.%s" % ("ques_train", hparams.src)
  tgt_file = "%s.%s" % (hparams.train_prefix, hparams.tgt)
  feature_file_s = "%s.%s" % ("feature_s_train", hparams.src)
  feature_file_e = "%s.%s" % ("feature_e_train", hparams.src)
  ...
  with graph.as_default(), tf.container(scope or "train"):
    ...
    ques_dataset = tf.data.TextLineDataset(ques_file)
    feature_dataset_s = tf.data.TextLineDataset(feature_file_s)
    feature_dataset_e = tf.data.TextLineDataset(feature_file_e)
...
class EvalModel(
    collections.namedtuple("EvalModel",
                           ("graph", "model", "src_file_placeholder",
                            "tgt_file_placeholder", "ques_file_placeholder",
                            "feature_file_s_placeholder", "feature_file_e_placeholder",
                            "iterator"))):
  pass
...
def create_eval_model(model_creator, hparams, scope=None, extra_args=None):
  ...
  with graph.as_default(), tf.container(scope or "eval"):
    ...
    ques_file_placeholder = tf.placeholder(shape=(), dtype=tf.string)
    feature_file_s_placeholder = tf.placeholder(shape=(), dtype=tf.string)
    feature_file_e_placeholder = tf.placeholder(shape=(), dtype=tf.string)
    
    src_dataset = tf.data.TextLineDataset(src_file_placeholder)
    tgt_dataset = tf.data.TextLineDataset(tgt_file_placeholder)
    ques_dataset = tf.data.TextLineDataset(tgt_file_placeholder)
    feature_dataset_s = tf.data.TextLineDataset(feature_file_s_placeholder)
    feature_dataset_e = tf.data.TextLineDataset(feature_file_e_placeholder)
  ...
  return EvalModel(
      ...
      ques_file_placeholder=ques_file_placeholder,
      feature_file_s_placeholder=feature_file_s_placeholder,
      feature_file_e_placeholder=feature_file_e_placeholder,
      iterator=iterator)
...
class InferModel(
    collections.namedtuple("InferModel",
                           ("graph", "model", "src_placeholder",
                            "ques_placeholder",
                            "feature_s_placeholder", "feature_e_placeholder",
...
def create_infer_model(model_creator, hparams, scope=None, extra_args=None):
  ...
  with graph.as_default(), tf.container(scope or "infer"):
    ...
    ques_placeholder = tf.placeholder(shape=[None], dtype=tf.string)
    feature_s_placeholder = tf.placeholder(shape=[None], dtype=tf.string)
    feature_e_placeholder = tf.placeholder(shape=[None], dtype=tf.string)

    batch_size_placeholder = tf.placeholder(shape=[], dtype=tf.int64)

    src_dataset = tf.data.Dataset.from_tensor_slices(src_placeholder)
    ques_dataset = tf.data.Dataset.from_tensor_slices(ques_placeholder)
    feature_s_dataset = tf.data.Dataset.from_tensors(feature_s_placeholder)
    feature_e_dataset = tf.data.Dataset.from_tensors(feature_e_placeholder)
    #feature_s_placeholder = tf.Print(feature_s_placeholder,[feature_s_placeholder])
    """
    feature_dataset_s = tf.data.Dataset.from_tensor_slices(feature_s_placeholder)
    feature_dataset_e = tf.data.Dataset.from_tensor_slices(feature_e_placeholder)
    """
  ...
  return InferModel(
      ...
      feature_s_placeholder=feature_s_placeholder,
      feature_e_placeholder=feature_e_placeholder,
