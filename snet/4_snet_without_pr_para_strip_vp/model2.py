...
# this model adjusts yp1 and yp2
...
class Model(object):
	def __init__(self, config, batch, word_mat=None, char_mat=None, trainable=True, opt=True):
		...
		self.c, self.q, self.ch, self.qh, self.y1, self.y2, self.qa_id, \
			self.c_pr, self.ch_pr, self.pr, self.para_count, self.answer_info, \
			self.passage_len = batch.get_next()
	...
	def get_vP(self,i,att_vP,q_,answer_info,y1,y2,c_pr_mask,cmax_c,clen_c,seq_mask):
		# max para limit
		config = self.config
		...
		if opt:
			...
			def b3(): 
				seq_mask_ = tf.sequence_mask(c_len,maxlen=c_maxlen)
				#seq_mask_ = tf.Print(seq_mask_,[tf.shape(seq_mask_)],message="SEQ")
				return c_maxlen_int, c_len_int, seq_mask_
			def b4():
				print(clen_c.get_shape(),c_len_int.get_shape())
				a = tf.concat([cmax_c, c_maxlen_int],axis=0)
				b = tf.concat([clen_c, c_len_int],axis=1)
				seq_mask_para = tf.sequence_mask(c_len,maxlen=c_maxlen)
				seq_concat = tf.concat([seq_mask,seq_mask_para],axis=1)
				#seq_concat = tf.Print(seq_concat,[tf.shape(seq_concat)],message="SEQconcat")

				return a,b,seq_concat
			cmax_c, clen_c, seq_mask = tf.cond(tf.equal(i, zero), b3, b4)
		...
		return tf.add(i,tf.constant(1)), att_vP, q__,answer_info,y1,y2,c_pr_mask,cmax_c,clen_c,seq_mask
	
	def condition(self,i,att_vP,q,answer_info,y1,y2,c_pr_mask,cm,cl,seq_mask):
	...
	def ready(self):
		...
		self.rnn2 = gru(num_layers=1, num_units=d, batch_size=N, input_size=1800,\
			keep_prob=config.keep_prob, is_train=self.is_train)
		"""
		result, self.att_vP, q, self.answer_info, \
			self.y1, self.y2, self.c_pr_mask, cmax_c, clen_c, self.seq_mask = \
			tf.while_loop(self.condition, self.get_vP,
				loop_vars=[self.i, self.att_vP, qtemp, self.answer_info, self.y1, self.y2,
					c_pr_mask, cmax_c, clen_c, seq_mask],
				shape_invariants= [
					self.i.get_shape(), tf.TensorShape([N, None, 2*d]),
					tf.TensorShape([N, None, 900]), self.answer_info.get_shape(),
					tf.TensorShape([None, None]), tf.TensorShape([None, None]), 
					tf.TensorShape([None, None]), tf.TensorShape([None]),
					tf.TensorShape([N, None]), tf.TensorShape([None, None])])

		# vp_mask_count = tf.reduce_sum(tf.cast(self.seq_mask,tf.int32),axis=1)	
		# the line above is the same as below, I hope
		vp_mask_count = tf.reduce_sum(clen_c, axis=1)
		...
		vp_mask_max_count = tf.reduce_sum(cmax_c)
		vp_final_pad_meta = vp_mask_max_count_c_like - vp_mask_count

		#vp_final_pad_seq = tf.sequence_mask(vp_final_pad_meta, maxlen=vp_mask_max_count)
		vp_final_pad_seq = tf.sequence_mask(vp_final_pad_meta)
		self.seq_mask = tf.concat([self.seq_mask,vp_final_pad_seq],axis=1)

		#vp_final_pad_seq = tf.reverse(vp_final_pad_seq, axis=[1]) 
		#vp_mask = tf.logical_or(self.seq_mask,vp_final_pad_seq)

		pad_length = tf.reduce_max(vp_final_pad_meta)
		paddings = tf.convert_to_tensor([[0, 0], [0, pad_length], [0, 0]])
		new_vp = tf.pad(self.att_vP, paddings, "CONSTANT")
		new_vp = tf.reshape(tf.boolean_mask(new_vp, self.seq_mask), [N, vpmccl, 2*config.hidden])

		#vp_mask_len = tf.reduce_sum(tf.cast(vp_mask,tf.int32),axis=1)
		seq_mask_len = tf.reduce_sum(tf.cast(self.seq_mask,tf.int32),axis=1)

		#att_vP = tf.boolean_mask(self.att_vP, vp_mask)

		"""
		self.att_vP = tf.Print(self.att_vP,["self.seq_mask",tf.shape(self.seq_mask),
			"self.c",tf.shape(self.c),
			"self.att_vp",tf.shape(self.att_vP),"new_vp",tf.shape(new_vp)],message="SEQMASK")
		q = tf.Print(q,["vp_mask_max_count",vp_mask_max_count,
			"seq_mask_len",seq_mask_len,
			"self.c_len",self.c_len,"vp_mask_count",vp_mask_count,
			],message="SEQMASK+PAD",summarize=N)
		"""

		self.att_vP = new_vp
		#self.q_mask = tf.Print(self.q_mask,["passage_len",self.passage_len,
		#	"c_len_concat",clen_c,
		#	],message="SEQMASK+PAD",summarize=N*config.max_para)
		with tf.variable_scope("pointer"):
			...
			logits1, logits2 = pointer(init, self.att_vP, d, self.c_mask)
		...
		with tf.variable_scope("predict"):
			...
			#self.yp1 = tf.Print(self.yp1,[self.yp1],message="yp1",summarize=20)
			#self.yp2 = tf.Print(self.yp2,[self.yp2],message="yp2",summarize=20)
			
			#losses = tf.Print(losses,[losses,tf.reduce_sum(logits1),tf.reduce_sum(logits2),
			#	tf.reduce_sum(self.y1),tf.reduce_sum(self.y2)],
			#	message="losses",summarize=20)
