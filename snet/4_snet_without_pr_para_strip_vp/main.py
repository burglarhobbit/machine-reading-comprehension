...
def train(config):
	...
	print(g.get_collection_ref(key))
	loss_save = 100.0
	patience = 0
	lr = config.init_lr
		...
		for _ in tqdm(range(1, config.num_steps + 1)):
			...
			if config.with_passage_ranking:
				if global_step % config.period == 0:					
					...
					loss_esp, loss_pr, loss_ee, train_op_ee, _ = sess.run(
						[model.loss, model.pr_loss, model.e_loss, model.train_op_ee,
						model.seq_mask],
						feed_dict={ handle: train_handle})
					#print(loss_esp)
				...
				if global_step % config.period == 0:
					...
					#print(loss_esp)
			# eval
			if global_step % config.checkpoint == 0 or global_step in [100,500]:
