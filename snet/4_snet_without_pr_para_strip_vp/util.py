	...
	def parse(example):
		...
		features = tf.parse_single_example(example,
										   features={
											   ...
											   "answer_info": tf.FixedLenFeature([], tf.string),
											   "id": tf.FixedLenFeature([], tf.int64),
											   "passage_count": tf.FixedLenFeature([], tf.int64),
											   "passage_len": tf.FixedLenFeature([], tf.string),
										   })
		passage_idxs = tf.reshape(tf.decode_raw(
			features["passage_idxs"], tf.int32), [config.max_para*para_limit])
		...
		passage_char_idxs = tf.reshape(tf.decode_raw(
			features["passage_char_idxs"], tf.int32), [para_limit*config.max_para, char_limit])
		...
		y1 = tf.reshape(tf.decode_raw(
			features["y1"], tf.float32), [para_limit*config.max_para])
		#print(tf.shape(y1))
		y2 = tf.reshape(tf.decode_raw(
			features["y2"], tf.float32), [para_limit*config.max_para])
		qa_id = features["id"]
		passage_count = features["passage_count"]
		answer_info = tf.reshape(tf.decode_raw(features["answer_info"], tf.int32), [3])
		passage_len = tf.reshape(tf.decode_raw(features["passage_len"], tf.int32), 
			[config.max_para])
		...
		return passage_idxs, ques_idxs, passage_char_idxs, ques_char_idxs, y1, y2, qa_id,\
			   passage_pr_idxs, passage_char_pr_idxs, passage_rank, passage_count, answer_info, \
			   passage_len
	return parse
...
def get_batch_dataset(record_file, parser, config):
	...
	if config.is_bucket:
		...
		def key_func(passage_idxs, ques_idxs, passage_char_idxs, ques_char_idxs, y1, y2, qa_id, \
					passage_pr_idxs, passage_pr_char_idxs, passage_rank, para_count, answer_info):
			c_len = tf.reduce_sum(
...
def convert_tokens(config, eval_file, qa_id, pp1, pp2):
	...
	for qid, p1, p2 in zip(qa_id, pp1, pp2):
		...
		extracted_answer = passage_pr_concat_[p1:p2+1]
