...
a = tf.random_normal_initializer(stddev=0.1)

init_fw1 = tf.get_variable("init_fw",shape=[1, 8, 150],initializer=
					tf.zeros_initializer())
init_fw2 = tf.zeros(shape=[1, 8, 150])
class cudnn_gru:
	...
	def __call__(self, inputs, seq_len, keep_prob=1.0, is_train=None, concat_layers=True):
		...
		for layer in range(self.num_layers):
			...
			with tf.variable_scope("fw_{}".format(layer)):
				out_fw, _ = gru_fw(
					outputs[-1] * mask_fw, initial_state=(init_fw2, ))
...
## made this function using dot_attention
## inputs = rQ, memory = vp
def pr_attention(inputs, memory, mask, hidden, name_scope,
				  keep_prob=1.0, is_train=None, scope="dot_attention"):
	with tf.name_scope(name_scope):
		with tf.variable_scope(scope):
			inputs = tf.tile(tf.expand_dims(inputs, axis=1), [
			...
			with tf.variable_scope("attention"):
				...
				mask = tf.tile(tf.expand_dims(mask, axis=1), [1, JX, 1])
				logits = tf.nn.softmax(softmax_mask(outputs, mask))
				outputs = tf.matmul(logits, memory) #rP
				res = tf.concat([inputs, outputs], axis=2)
				
			with tf.variable_scope("passage_scores"):
				dim = res.get_shape().as_list()[-1]
				d_res = dropout(res, keep_prob=keep_prob, is_train=is_train)
				g = tf.nn.tanh(dense(d_res, dim, use_bias=False, name_scope="gate_layer"))
				return g

def summ2(memory, inputs, max_para, hidden, mask, keep_prob=1.0, is_train=None, scope="summ"):
	with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):

		inputs_ = tf.tile(tf.expand_dims(inputs, axis=1), [
				1, tf.shape(memory)[1], 1]) # as rQ is of rank 2 but memory(vp is of rank 3)

		d_memory = dropout(memory, keep_prob=keep_prob, is_train=is_train)
		inputs_ = dropout(inputs_, keep_prob=keep_prob, is_train=is_train)
		

		s0_d = dense(d_memory, hidden, scope="s0", name_scope="summ_layer_s0")
		s0_i = dense(inputs_, hidden, scope="s0", name_scope="summ_layer_s0")
		s0 = tf.nn.tanh(s0_d + s0_i)
		
		s = dense(s0, 1, use_bias=False, scope="s", name_scope="summ_layer_s")
		s1 = softmax_mask(tf.squeeze(s, [2]), mask)
		a = tf.expand_dims(tf.nn.softmax(s1), axis=2)
		res = tf.reduce_sum(a * memory, axis=1) # rP
		#res = tf.Print(res,[tf.shape(res)],"res")
		res = tf.concat([inputs, res],axis=1)

	with tf.variable_scope("passage_scores"):
		dim = res.get_shape().as_list()[-1]
		d_res = dropout(res, keep_prob=keep_prob, is_train=is_train)

		g = tf.nn.tanh(dense(d_res, 1, use_bias=False, name_scope="gate_layer"))
		return g
		#return res
