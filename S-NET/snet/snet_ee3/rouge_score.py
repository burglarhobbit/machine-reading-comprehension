...
def _len_lcs(x, y):
  ...
  table = _lcs(x, y)
  n, m = len(x), len(y)
  return table["start_end"], table[n, m]
...
def _lcs(x, y):
  ...
  while i > 0 and j > 0:
    if x[i-1] == y[j-1]:
      ...
      j-=1
  table["start_end"] = (start_index, end_index)
  return table
...
def rouge_l_sentence_level(evaluated_sentences, reference_sentences):
  ...
  index, lcs = _len_lcs(evaluated_words, reference_words)
  return index,_f_p_r_lcs(lcs, m, n)
...
def rouge(hypotheses, references):
  ...
  for hyp, ref in zip(hypotheses, references):
    index,rouge_score = rouge_l_sentence_level([hyp], [ref])
    rouge_l.append(rouge_score)

  return rouge_1, rouge_2, rouge_l, index