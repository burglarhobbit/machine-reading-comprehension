...
#target_dir = "data"
target_dir = os.path.join(path, "preprocess", "snet", "msmarco", "pr_para_strip")
log_dir = os.path.join(path, "snet", "snet_without_pr_para_strip_vp", "log", "event")
save_dir = os.path.join(path, "snet", "snet_without_pr_para_strip_vp", "log", "model")
save_dir_temp = os.path.join(path, "snet", "snet_without_pr_para_strip_vp", "log", "model")
answer_dir = os.path.join(path, "snet", "snet_without_pr_para_strip_vp", "log", "answer")
...
flags.DEFINE_string("gpu_id", "1", "gpu id to use for training")
...
flags.DEFINE_integer("test_para_limit", 400, 
					 "Limit length for paragraph in test file") # 1000
...
flags.DEFINE_boolean("use_cudnn_gru", False, "Whether to use cudnn rnn (should be False for CPU)")
flags.DEFINE_boolean("is_bucket", False, "build bucket batch iterator or not")
flags.DEFINE_boolean("line_limit_prepro", False, "limit prepro to limited number of lines for POC")
flags.DEFINE_integer("line_limit", 100, "limit prepro to limited number of lines for POC")
flags.DEFINE_boolean("restore_checkpoint", False, "Restore Checkpoint")
...
flags.DEFINE_float("init_lr", 1.0, "Initial learning rate for Adadelta") #1.0
