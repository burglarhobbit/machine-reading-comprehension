...
def create_train_model(
  ...
  feature_file = "%s.%s" % ("feature_train", hparams.src)
  ...
  with graph.as_default(), tf.container(scope or "train"):
    ...
    feature_dataset = tf.data.TextLineDataset(feature_file)
...
class EvalModel(
    collections.namedtuple("EvalModel",
                           ("graph", "model", "src_file_placeholder",
                            ...
                            "feature_file_placeholder", "iterator"))):
  pass
...
def create_eval_model(model_creator, hparams, scope=None, extra_args=None):
  ...
  with graph.as_default(), tf.container(scope or "eval"):
    ...
    feature_file_placeholder = tf.placeholder(shape=(), dtype=tf.string)
    src_dataset = tf.data.TextLineDataset(src_file_placeholder)
    tgt_dataset = tf.data.TextLineDataset(tgt_file_placeholder)
    ques_dataset = tf.data.TextLineDataset(ques_file_placeholder)
    feature_dataset = tf.data.TextLineDataset(feature_file_placeholder)
    iterator = iterator_utils.get_iterator(
        src_dataset,
        tgt_dataset,
        ques_dataset,
  ...
  return EvalModel(
      ...
      feature_file_placeholder=feature_file_placeholder,
      iterator=iterator)


class InferModel(
    collections.namedtuple("InferModel",
                           ("graph", "model", "src_placeholder", "ques_placeholder",
                            "feature_placeholder", "batch_size_placeholder", "iterator"))):
  pass
...
def create_infer_model(model_creator, hparams, scope=None, extra_args=None):
  ...
  with graph.as_default(), tf.container(scope or "infer"):
    ...
    feature_placeholder = tf.placeholder(shape=[None], dtype=tf.string)
    batch_size_placeholder = tf.placeholder(shape=[], dtype=tf.int64)

    src_dataset = tf.data.Dataset.from_tensor_slices(
        src_placeholder)
    ques_dataset = tf.data.Dataset.from_tensor_slices(
        ques_placeholder)
...
def _create_or_load_embed(embed_name, vocab_file, embed_file,
  ...
  if vocab_file and embed_file:
    utils.print_out("# EMBEDDING EXISTS")
    embedding = _create_pretrained_emb_from_txt(vocab_file, embed_file)
  else:
    with tf.device(_get_embed_device(vocab_size)):
      utils.print_out("# EMBEDDING DOES NOT EXISTS")
