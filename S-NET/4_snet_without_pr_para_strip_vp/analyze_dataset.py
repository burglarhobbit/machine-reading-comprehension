...
def process_file(config, max_para_count, filename, data_type, word_counter, char_counter, is_line_limit, rouge_metric):
	...
	for i in tqdm(range(total_lines)):
		...
		passage_concat = ''
		passage_count = 0 # total passage count
		...
		temp_token_count = 0
		## passage index, answer start, answer end {w.r.t passage with index number}
		answer_info = [0]*3 
		...
		for j,passage in enumerate(source['passages']):
			passage_text = passage['passage_text'].replace(
				"''", '" ').replace("``", '" ').lower()
			passage_concat += passage_text.strip() + " "
			passage_token = word_tokenize(passage_text.strip())
			passage_pr_tokens[j] = passage_token
			passage_pr_single_token[j][:len(passage_token)] = passage_token
			passage_count += 1
			temp_token_count += len(passage_token)
			passage_rank[j] = passage['is_selected']
			length = len(passage_pr_tokens[j])
			passage_len.append(len(passage_token))

			if length>config.max_para:
				para_length_exceeded += 1
			if length>max_para_length:
				max_para_length = length
		passage_concat = passage_concat.strip()
		passage_tokens = word_tokenize(passage_concat)
		assert temp_token_count == len(passage_tokens)
			...
			for answer_k,k in enumerate(answer):
				...
				for l, passage in enumerate(passage_pr_tokens):
					...
					if fpr_scores[rouge_metric]>highest_rouge_l[rouge_metric]:
						...
						passage_rank_index = l # will not be used due to using is_selected param
						answer_info = [l,index[0],index[-1]]
		...
		example = {"passage_tokens": passage_tokens, "passage_chars": passage_chars, "ques_tokens": ques_tokens,
				   ...
				   "passage_pr_chars": passage_pr_chars, "passage_count": passage_count,
				   "passage_len": passage_len, "answer_info": answer_info}
		...
		eval_examples[str(total)] = {
			...
			"passage_rank": passage_rank, "passage_len": passage_len}
		line = fh.readline()
		if total%1000 == 0:
...
def build_features(config, examples, data_type, out_file, word2idx_dict, char2idx_dict, is_test=False):
	...
	def filter_func(example, is_test=False):
		return len(example["passage_tokens"]) > para_limit*config.max_para or len(example["ques_tokens"]) > ques_limit
	...
	for example in tqdm(examples):
		...
		total += 1
		passage_idxs = np.zeros([para_limit*config.max_para], dtype=np.int32)
		passage_rank = np.array(example["passage_rank"], dtype=np.float32)
		passage_char_idxs = np.zeros([para_limit*config.max_para, char_limit], dtype=np.int32)
		...
		answer_info = np.array(example["answer_info"], dtype=np.int32)
		passage_len = np.zeros([config.max_para], dtype=np.int32)
		passage_len[:len(example["passage_len"])] = example["passage_len"]
		...
		for i, paragraph in enumerate(example["passage_pr_tokens"]):
			for j, token in enumerate(paragraph):
				passage_pr_idxs[i*para_limit+j] = _get_word(token)
		# for pr
		for i, paragraph in enumerate(example["passage_pr_chars"]):
			for j, token in enumerate(paragraph):
				for k, char in enumerate(token):
					if k == char_limit:
						break
					passage_char_pr_idxs[i*para_limit+j, k] = _get_char(char)

		start, end = example["y1s"][-1], example["y2s"][-1]
		
		# overrided with answer_info variable, will update this variable during training
		y1[start], y2[end] = 1.0, 1.0 

		#print(example["passage_count"])
		record = tf.train.Example(features=tf.train.Features(feature={
								  ...
								  "answer_info": tf.train.Feature(bytes_list=tf.train.BytesList(value=[answer_info.tostring()])),
								  "id": tf.train.Feature(int64_list=tf.train.Int64List(value=[example["id"]])),
								  "passage_count": tf.train.Feature(int64_list=tf.train.Int64List(value=[example["passage_count"]])),
								  "passage_len": tf.train.Feature(bytes_list=tf.train.BytesList(value=[passage_len.tostring()])),
								  }))
