...
#target_dir = "data"
model = "snet"
title = "snet_refactored_from_rnet"
target_dir = os.path.join(path, "preprocess", model, "msmarco", title)
log_dir = os.path.join(path, model, title, "log", "event")
save_dir = os.path.join(path, model, title, "log", "model")
save_dir_temp = os.path.join(path, model, title, "log", "model")
answer_dir = os.path.join(path, model, title, "log", "answer")
...
flags.DEFINE_string("test_data", "test", "To test on which dataset? train | dev | test")

flags.DEFINE_string("gpu_id", "0", "gpu id to use for training")
...
flags.DEFINE_integer("max_para", 10, "No of paragraphs")
flags.DEFINE_integer("single_para_limit", 400, "No of paragraphs")
flags.DEFINE_integer("ques_limit", 50, "Limit length for question")
flags.DEFINE_integer("test_para_limit", 1500, 
					 "Limit length for paragraph in test file") #1000
flags.DEFINE_integer("test_ques_limit", 50,
...
flags.DEFINE_boolean("use_gpu", False, "Whether to use gpu or cpu")
...
flags.DEFINE_integer("line_limit", 100, "limit prepro to limited number of lines")
