	...
	def parse(example):
		...
		features = tf.parse_single_example(example,
										   features={
											   ...
											   "passage_count": tf.FixedLenFeature([], tf.int64),
										   })
		...
		passage_pr_idxs = tf.reshape(tf.decode_raw(
			features["passage_pr_idxs"], tf.int32), [max_para*spl])
		passage_pr_char_idxs = tf.reshape(tf.decode_raw(
			features["passage_pr_char_idxs"], tf.int32), [max_para*spl, char_limit])
		...
		qa_id = features["id"]
		passage_count = features["passage_count"]
		return passage_idxs, ques_idxs, passage_char_idxs, ques_char_idxs, y1, y2, qa_id, \
			passage_pr_idxs, passage_pr_char_idxs, passage_count
	return parse
...
def make_dataset_for_ans_syn(dataset, eval_file, data_type, qa_id, pp1, pp2):
	answer_dict = {}
	remapped_dict = {}
	outlier = False
	...
	# passage.target = evidence extraction output (span)
	# ground truth = groud truth answer from the dataset
	ques_file   	  = open(o+'ques_%s.%s'       %(data_type,suffix1),'w')
	ans_file	      = open(o+'passage_%s.%s'    %(data_type,suffix2),'w')
	ground_t_file     = open(o+'ground_t_%s.%s'   %(data_type,suffix2),'w')
	feature_file 	  = open(o+'feature_%s.%s'    %(data_type,suffix1),'w')
	passage_file 	  = open(o+'passage_%s.%s'    %(data_type,suffix1),'w')

	line = fh.readline()
	uuid_dict = {} # for uuid to query
	while line:
		source = json.loads(line)
		uuid = source['query_id']
		#dict1 = { 'query': source['query'].lower()}
		passage_list = []
		for j,passage in enumerate(source['passages']):
			passage_list.append(passage['passage_text'].lower())
		#dict2 = {'passage': passage_list}
		uuid_dict[uuid] = { 'query': source['query'].lower(), 'passage': passage_list}
		line = fh.readline()
	...
	for qid, p1, p2 in zip(qa_id, pp1, pp2):
		...
		spans = eval_file[str(qid)]["spans"]
		uuid = eval_file[str(qid)]["uuid"]
		ground_truth = eval_file[str(qid)]["answers"][0]
		spans_l = len(spans)
	
		ques = uuid_dict[uuid]['query']

		start_idx = spans[p1][0]
		end_idx = spans[p2][1]
		extracted_answer = passage_concat[start_idx:end_idx]
		count = 0
		extracted_ans_syn_passage = ""
		rouge_l = rouge_get_scores(extracted_answer, ground_truth)
		if rouge_l[0] < 0.5:
			continue
		for j,passage in enumerate(uuid_dict[uuid]['passage']):
			count += len(passage)
			if count >= start_idx:
				extracted_ans_syn_passage = " ".join(word_tokenize(passage))
				break

		answer_dict[str(qid)] = extracted_answer
		remapped_dict[uuid] = extracted_answer

		print(ques,file=ques_file)
		print(" ".join(word_tokenize(extracted_answer)),file=ans_file)
		print(" ".join(word_tokenize(ground_truth)),file=ground_t_file)
		print(str(start_idx) + " " + str(end_idx),file=feature_file)
		print(extracted_ans_syn_passage,file=passage_file)
