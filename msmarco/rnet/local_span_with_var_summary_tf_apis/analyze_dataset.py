...
def process_file(max_para_count, filename, data_type, word_counter, char_counter, is_line_limit, rouge_metric):
	...
	for i in tqdm(range(total_lines)):
		...
		example = {"passage_tokens": passage_tokens, "passage_chars": passage_chars,
					"ques_tokens": ques_tokens, "ques_chars": ques_chars,
					"y1s": y1s, "y2s": y2s, "id": total, "uuid": source["query_id"],}
		examples.append(example)
		eval_examples[str(total)] = {
			"passage_concat": passage_concat, "spans": spans, "answers": answer_texts,
			"uuid": source["query_id"], "question": ques}
		line = fh.readline()
		if total%1000 == 0:
