			...
			for para in article["paragraphs"]:
				...
				for qa in para["qas"]:
					...
					example = {"context_tokens": context_tokens, "context_chars": context_chars,
						...
						"y1s": y1s, "y2s": y2s, "id": total, "uuid": qa["id"]}
					examples.append(example)
					eval_examples[str(total)] = {
						"context": context, "spans": spans, "answers": answer_texts,
						"uuid": qa["id"], "question":ques, "y1s": y1s[-1], "y2s": y2s[-1]}
...
def prepro(config):
	...
	word_emb_file = config.fasttext_file if config.fasttext else config.glove_file
