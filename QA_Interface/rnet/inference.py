...
# target_dir = "data"
# save_dir = "log/model"
path = "/media/hdd2/snetP_data"
target_dir = os.path.join(path, "preprocess", "rnet", "squad")
save_dir = os.path.join(path, "rnet", "squad", "log_new2", "model")
...
class InfModel(object):
    ...
    def __init__(self, word_mat, char_mat):
        self.c = tf.placeholder(tf.int32, [64, None])
        self.q = tf.placeholder(tf.int32, [64, None])
        self.ch = tf.placeholder(tf.int32, [64, None, char_limit])
        self.qh = tf.placeholder(tf.int32, [64, None, char_limit])
...
class Inference(object):
    ...
    def response(self, context, question):
        ...
        yp1, yp2 = sess.run([model.yp1, model.yp2], feed_dict={
                            model.c: [context_idxs[0,:]]*64, model.q: [ques_idxs[0,:]]*64, model.ch: [context_char_idxs[0,:]]*64, model.qh: [ques_char_idxs[0,:]]*64})
        # start_idx = span[yp1[0]][0]
        # end_idx = span[yp2[0]][1]
        start_idx = span[yp1[0]][0]
        end_idx = span[yp2[0]][1]
        return start_idx, end_idx, context[start_idx: end_idx]
...
def get_answer():
    ...
    start, end, ans = infer.response(context, question)

    dict = {'start': start, 'end': end, 'ans':ans}
...
if __name__ == "__main__":
    ...
    start1, end1, ans1 = infer.response(context, ques1)
    print("Answer 1: {}".format(ans1))

    # Correct: graupel, Output: graupel
    start2, end2, ans2 = infer.response(context, ques2)
    print("Answer 2: {}".format(ans2))

    # Correct: within a cloud, Output: within a cloud
    start3, end3, ans3 = infer.response(context, ques3)
